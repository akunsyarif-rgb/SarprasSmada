...
1. **Domain service DILARANG memanggil `SpreadsheetApp` secara langsung.** Ini berlaku untuk seluruh domain: `apps-script/users/`, `apps-script/master-data/`, `apps-script/reports/`, dan `apps-script/audit/`. Seluruh kebutuhan baca/tulis/cari data pada domain-domain tersebut wajib menggunakan fungsi generik yang disediakan `core/DatabaseService.gs` (`getSheetByName`, `getAllRows`, `getRowById`, `findRows`, `insertRow`, `updateRowById`).
...
Audit kepatuhan terhadap aturan ini dapat dilakukan dengan mencari (grep) penggunaan `SpreadsheetApp` di luar folder `apps-script/core/` — hasil pencarian tersebut harus kosong.
